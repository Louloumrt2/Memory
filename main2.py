...
class Card:
    def __init__(self, x, y, name, image, color, size, location = "play"):
        ...
        # Effets spéciaux
        self.time_with_saignement_acc = 0
    ...
    def update(self):
        ...
        if any(tag[0]=="saignement" for tag in self.tags) : self.time_with_saignement_acc 
        
    def put_tag(self, tag) :
        global selection_overload
    ...
    def activate_effect(self, nb_match, selection, lesnoms, cards, lvl, already_done, nb_move) :
        ...
        if lvl :
            match self.name :
                ...
                case "Croqueur_Avide" :
                    if selection.count(self) < len(selection):
                        target = self 
                        while target == self :
                            target = random.choice(selection)
                        
                        target.put_tag(("saignement", lvl, (255,0,0)))
        ...
        for tag in self.tags :
            ...
            if "saignement" == tag[0] :
                    if not selection_locked :
                        if self.time_with_saignement_acc >= 1:
                            quantity_to_delete = min(tag[1], int(self.time_with_saignement_acc))
                            self.time_with_saignement_acc -= quantity_to_delete
                            add_score(-2*quantity_to_delete)
                            removed_tags.add(tag)
                            if tag[1] - quantity_to_delete > 0 : added_tag.add(("saignement", tag[1]-quantity_to_delete, tag[2]))

        
        self.tags |= added_tag
        self.tags -= removed_tags

        if all(tag[0] != "saignement" for tag in self.tags) : 
            selection_overload -= 1
...
def play_memory(num_pairs=8, lives=3, forced_cards = None):
    ...
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if not selection_locked :
                    if getattr(event, "button", None) == 2 : # touche de pré-selection (clique molette)
                        for card in cards:
                            if card.rect.collidepoint(event.pos) and not card.matched and not card.flipped and not card.remove and (card not in selection) and not any(tag[0]=="saignement" for tag in card.tags):
                                if card.selected :
                                    card.selected = False
                                    selection.remove(card)
                                elif len(selection) <(SELECTION_LIMIT + selection_overload):
                                    card.selected = True
                    ...
                    if getattr(event, "button", None) == 3 : # touche de déselection (clique droit)
                        for card in cards:
                            if not card.flipped and not card.remove and card.selected and not any(tag[0]=="saignement" for tag in card.tags) :
                                if card.selected :
                                    card.selected = False
                                    selection.remove(card)
                    ...
                    if getattr(event, "button", None) == 1 : # touche de révellation (montre une carte : clic gauche)
                        for card in cards:
                            ...
                            if not card in selection and not card.selected and len(selection) < (SELECTION_LIMIT + selection_overload) and card.rect.collidepoint(event.pos)  :
                                card.selected = True 
                                selection.append(card)
                                card.flipped = True
                                last_selection_time = pygame.time.get_ticks()
        ...
        # logique du jeu
        if len(selection) == (SELECTION_LIMIT + selection_overload):
            selection_locked = True
            for card in selection :
                card.flipped=True
